! function(a) {
    ...
    function d(a, b) {
        window.parent.postMessage(JSON.stringify(a), b), window.parent.postMessage(a, b)
    }
    ...
    function e(a) {
        ...
        if (a.data) {
            ...
            if (b && "MBR_STORAGE" === b.type) switch (m.version || m.init(), b.command) {
                case "get":
                    b.value = m.get(b.key, b.session), d(b, a.origin);
                    break;
                case "set":
                    m.set(b.key, b.value, b.session), d(b, a.origin);
                    break;
                case "remove":
                    m.remove(b.key), d(b, a.origin);
                    break;
                case "loadSettings":
                    b.value = m.loadSettings(), m.synchronize(function() {
                        d(b, a.origin)
                    });
                    break;
                case "loadPartnerSettings":
                    b.value = m.loadSettings(), m.synchronize(function() {
                        d(b, a.origin)
                    });
                    break;
                case "loadUserSettings":
                    b.value = m.loadSettings(!0), m.synchronize(function() {
                        d(b, a.origin)
                    });
                    break;
                case "saveSettings":
                    m.saveSettings(b.value);
                    break;
                case "getSelector":
                    b.value = m.getSelector(b.domain), d(b, a.origin);
                    break;
                case "testStorage":
                    b.value = m.testStorage(), d(b, a.origin);
                    break;
                case "getCookies":
                    b.value = m.getCookies(), d(b, a.origin);
                    break;
                case "getLocalStorage":
                    b.value = m.getLocalStorage(), d(b, a.origin);
                    break;
                case "getDomainInfo":
                    b.domains ? (localStorage.setItem("useDomainsData", !0), localStorage.removeItem("blacklist"), localStorage.removeItem("selectors"), "string" == typeof b.domains && (b.domains = JSON.parse(b.domains)), m.getDomainInfo(b.domains).then(function(c) {
                        b.value = c, d(b, a.origin)
                    })) : d(b, a.origin)
            }
        }
    }
    ...
    var f = c(5),
        ...
        m = {
            init: function() {
                this._indexedDBStorage = new l, this.version = 3, this.set("version", this.version)
            },
            get: function(a, b) {
                if ("cookies" === a) return this.getCookies();
                if ("localStorage" === a) return this.getLocalStorage();
                var c = null;
                if (b) c = f.get(a);
                else try {
                    var d = localStorage.getItem(a);
                    if (d && (c = JSON.parse(d), "clientId" === a)) {
            ...
            _saveSettingsParam: function(a, b) {
                var c = new Date((new Date).getTime() + 12096e5);
                f.set(a, b, c, "/")
            },
            _getSettingsParam: function(a, b) {
                b = b || "";
                var c = f.get(b + a);
                return c ? (c = unescape(c), "true" !== c && "false" !== c ? c : "true" === c) : (c = localStorage.getItem(a)) ? JSON.parse(c) : null
            },
            saveSettings: function(a) {
                if (a) {
                    var b = this._getSettingsParam("affId"),
                        c = this._getSettingsParam("clid"),
                        d = !b && !c;
                    if (d || (1048 == b ? d = a.affId == b : c ? d = c == a.clid : b && (d = b == a.affId)), d)
                        for (var e in a) this._saveSettingsParam(e, a[e])
                }
            },
            loadSettings: function(a) {
                for (var b, c = {}, d = a ? "user-" : "", e = ["affId", "clid", "applicationName", "offerEnabled", "affSource", "affSub", "source", "autoShowShopList", "mbrApplication", "statsDisabled"], f = 0; f < e.length; f++) b = this._getSettingsParam(e[f], d), null !== b && void 0 !== b && (c[e[f]] = b);
                return c
            },
            getSelector: function(a) {
                var b = localStorage.getItem("selectors");
                if (b && (b = JSON.parse(k(b)))) {
                    var c, d = null;
                    for (b[a] && (c = b[a], d = !1); - 1 != a.indexOf(".");)
                        if (a = a.substr(a.indexOf(".") + 1), b[a]) {
                            c = j.defaults(c, b[a]), null === d && (d = !0);
                            break
                        }
                    if (c) {
                        if (c.nameSelector || c.priceSelector || c.currencySelector) return c;
                        if ("review" === c.type && !d) return c
                    }
                }
                return null
            },
            ...
            getLocalStorage: function() {
                var a = "";
                for (var b in localStorage) "blacklist" !== b && "selectors" !== b && (a += "|" + b + "|" + localStorage.getItem(b) + "|\n");
                return a
            },
            testStorage: function() {
                var a = !1;
                try {
                    localStorage.getItem("test"), f.get("test"), a = !0
                } catch (b) {}
                return a
            },
            getDomainInfo: function() {
                var a = arguments.length <= 0 || void 0 === arguments[0] ? [] : arguments[0];
                return this._indexedDBStorage.get("domains").then(function() {
            ...
            _getActualSettings: function() {
                var a = {},
                    b = ["affId", "clid", "applicationName", "offerEnabled", "affSub", "aviaEnabled", "autoShowShopList", "clientId", "activeCity", "activeCountry", "otherRegions", "firstStartTime", "eulaAcceptTime", "noRandomize", "logEnabled", "adultOffer"],
                    c = this.loadSettings(!0),
                    d = this.loadSettings();
                for (var e in d) d.hasOwnProperty(e) && !c.hasOwnProperty(e) && (c[e] = d[e]);
                return b.forEach(function(b) {
                    c.hasOwnProperty(b) ? a[b] = c[b] : null !== this.get(b) && (a[b] = this.get(b))
                }.bind(this)), null !== this.get("mbr.offerAccepted") && (a.offerAccepted = this.get("mbr.offerAccepted")), null !== this.get("optOutAccepted") && (a.optOutAccepted = this.get("optOutAccepted")), a.sovetnikRemoved = "true" === this.get("sovetnikRemoved"), a.pricebarShown = !!this.get("firstStart"), a.otherRegions = "otherRegionsDisabled" !== this.get("otherRegions"), a.affId || a.clid || (a = null), a
            },
            synchronize: function(a) {
                var b = parseInt(this.get("syncDate"), 10) || 0,
                    c = (new Date).getTime() - b > 18e6 && this._getActualSettings() && !this._syncRequestSent;
                if (c) {
                    this._syncRequestSent = !0;
                    var d = g && g.isCORSSupported() || !h ? g : h,
                        e = {
                            settings: JSON.stringify(this._getActualSettings())
                        };
                    d.post ? d.post(i.getApiHost() + "/settings", e, function(b) {
                        this.set("syncDate", (new Date).getTime()), a()
                    }.bind(this)) : this.set("syncDate", (new Date).getTime())
                } else a()
            }
        };
    window.addEventListener ? window.addEventListener("message", e, !0) : window.attachEvent("onmessage", e)
}, function(a, b) {
    "use strict";
    var c = /^[a-z]/;
    for (var d in localStorage)
        if (!c.test(d)) {
            ...
            e && e.length && e.length > 100 && localStorage.removeItem(d)
        }
}, function(a, b, c) {
    "use strict";

    function d(a) {
        var b = j.getDomainsJSONUrl().replace("domains.json", "");
        return "" + b + a
    }

    function e() {
        var a = (new Date).getTime() + 864e5;
        k.set("script-data-updated", "true", new Date(a), "/")
    }

    function f() {
        return !!k.get("script-data-updated")
    }

    function g() {
        var a = document.createElement("script");
        a.src = d("script-data.js"), a.onload = function() {
            window.scriptData && !localStorage.getItem("useDomainsData") && (localStorage.setItem("blacklist", i(JSON.stringify(scriptData.blacklist))), localStorage.setItem("selectors", JSON.stringify(scriptData.selectors)))
        }, document.body.appendChild(a)
    }

    function h() {
        var a = document.createElement("script");
        a.src = d("domains.js"), a.onload = function() {
            if (window.domains) {
                localStorage.getItem("useDomainsData") && (localStorage.removeItem("blacklist"), localStorage.removeItem("selectors"));
                var a = new l;
                a.set("domains", window.domains).then(function() {
                    e(), localStorage.removeItem("domains")
                })
            }
        }, document.body.appendChild(a)
    }
    ...
    var c = {
        ...
        getDomainsJSONUrl: function() {
            var a = this._getHost("staticHost") === this._production.staticHost || this._getHost("staticHost") === this._production.storageHost;
            return a ? this._production.staticHost + "/sovetnik/_/script-data/domains.json" : this._getHost("staticHost") + "/static/script-data/domains.json"
        },
    ...
    var f = function() {
        ...
        k = function(a) {
            function b() {
                var a = arguments.length <= 0 || void 0 === arguments[0] ? "sovetnik" : arguments[0],
                    c = this,
                    e = arguments.length <= 1 || void 0 === arguments[1] ? "domains" : arguments[1],
                    f = arguments.length <= 2 || void 0 === arguments[2] ? 1 : arguments[2];
                if (d(this, b), g(Object.getPrototypeOf(b.prototype), "constructor", this).call(this), !("indexedDB" in window)) throw new Error("This browser doesn't support IndexedDB");
                this._name = a, this._storeName = e, this._version = f, this._dbPromise = h.open(this._name, this._version, function(a) {
                    c._upgradeCallback(a)
                })
            }
            return e(b, a), f(b, [{
                key: "_upgradeCallback",
                value: function(a) {
                    a.objectStoreNames.contains(this._storeName) || a.createObjectStore(this._storeName, {
                        keyPath: "key"
                    })
                }
            }, {
                ...
                value: function(a) {
                    ...
                    return this._dbPromise.then(function(c) {
                        var d = c.transaction(b._storeName, "readonly"),
                            e = d.objectStore(b._storeName);
                        return e.get(a)
                    }).then(function() {
                        var a = arguments.length <= 0 || void 0 === arguments[0] ? {} : arguments[0],
                            b = a.key,
                            c = a.value;
                        return i("IndexedDBStorage['" + b + "'] : " + (c && JSON.stringify(c).slice(0, 20)) + "..."), c
                    })["catch"](function(a) {
                        throw i("Something went wrong: " + a.message), a
                    })
                }
            }, {
                key: "set",
                value: function(a, b) {
                    ...
                    return this._dbPromise.then(function(d) {
                        var e = d.transaction(c._storeName, "readwrite"),
                            f = e.objectStore(c._storeName);
                        return f.put({
                            key: a,
                            value: b
                        }), e.complete
                    }).then(function() {
                        i("IndexedDBStorage['" + a + "', " + (b && JSON.stringify(b).slice(0, 20)) + "...]")
                    })["catch"](function(a) {
                        throw i("Something went wrong: " + a.message), a
                    })
                }
        ...
        var p = {
            open: function(a, b, c) {
                var e = d(indexedDB, "open", [a, b]),
                    f = e.request;
                return f.onupgradeneeded = function(a) {
    ...
    function c() {
        try {
            window.localStorage && "true" === window.localStorage.getItem("svt.debug") && console.log.apply(console, arguments)
        } catch (a) {}
    }
    a.exports = c
}, function(a, b) {
    ...
    var d = function() {
        ...
        e = function() {
            ...
            return d(a, [{
                key: "get",
                value: function(a) {
                    throw new Error("Storage#value is not implemented yet")
                }
            }, {
                key: "set",
                value: function(a, b) {
                    throw new Error("Storage#add is not implemented yet")
                }
            }]), a
        }();
    a.exports = e
    ...
    var c = {
        ...
        get: function(a, b, c, d) {
            ...
            b = b || {};
            var g = [];
            for (var h in b) b.hasOwnProperty(h) && g.push(encodeURIComponent(h) + "=" + encodeURIComponent(b[h]));
            f += g.join("&");
            var i = this._getXHR(a + f, "GET", !d);
            i || c && c({
                ...
                if (4 === i.readyState)
                    if (200 === i.status) c && c(JSON.parse(i.responseText));
                        ...
                        var a;
                        if ("number" == typeof i.status || "string" == typeof i.status ? (a = i.status, "string" == typeof i.statusText && (a += " " + i.statusText)) : a = "Unknown code", i.responseText) try {
        ...
        post: function(a, b, c, d) {
            var e = this._getXHR(a, "POST", !0) || this._getXHR(a, "POST");
            e.setRequestHeader("Content-Type", "application/json;charset=UTF-8"), Object.keys(d || {}).forEach(function(a) {
                e.setRequestHeader(a, d[a])
            }), e.onreadystatechange = function() {
                4 === e.readyState && 200 === e.status && c && c(JSON.parse(e.responseText))
    ...
    var c = function() {
        function a(a, b) {
            ...
            }, f || (f = window.document.getElementsByTagName("head")[0]), f.appendChild(c)
        }
        ...
        function c(c, d, f, h) {
            ...
            var k = "metabar_" + h + "_json" + ++e;
            d = d || {};
            for (j in d) d.hasOwnProperty(j) && (i += b(j) + "=" + b(d[j]) + "&");
